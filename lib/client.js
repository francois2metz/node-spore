        ...
        if (this._isValidCall(method, callback, params, payload)) {
            ...
            try {
                for (var i in this.middlewares) {
                    if (this.middlewares[i].cond(method, request)) {
                        var retVal = this.middlewares[i].middleware(method, request);
            ...
            if (!response)
                ...
                this._endCall(method, response, callback, response_middlewares);
        }
    },
