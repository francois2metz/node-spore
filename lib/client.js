...
Client.prototype = {
    ...
    _call: function(methodName, args) {
        ...
        if (this._isValidCall(methodDef, callback, params, payload)) {
            var request = this._createRequest(methodDef, params, payload);
            var response_middlewares = [];
            var response = null;
            try {
                for (var i in this.middlewares) {
                    if (this.middlewares[i].cond(methodDef, request)) {
                        var retVal = this.middlewares[i].middleware(methodDef, request);
