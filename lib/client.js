...
function Client(middlewares, spec) {
    ...
    for (methodName in spec.methods) {
        if (spec.authentication === true
            && spec.methods[methodName].authentication !== false) {
            spec.methods[methodName].authentication = true;
        }
        if (spec.formats && !spec.methods[methodName].formats) {
            spec.methods[methodName].formats = spec.formats;
        }
