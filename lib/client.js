...
Client.prototype = {
    ...
    _call: function(methodName, args) {
        ...
        if (this._isValidCall(methodDef, callback, params, payload)) {
            ...
            try {
                for (var i in this.middlewares) {
                    ...
                    if (middleware.request) {
                        var response = middleware.request(methodDef, request);
                        if (response) {
                            break;
                        }
                    }
            ...
            if (!response)
                this._makeRequest(methodDef, request, callback);
            else
                this._endCall(methodDef, response, callback);
        }
    },

    _makeRequest: function(method, request, callback) {
        ...
        httpRequest.on('response', function(response) {
            ...
            response.on('end', function() {
                ...
                that._endCall(method, c, callback);
            });
        });
        httpRequest.end();
    },

    _endCall: function(method, response, callback) {
        try {
            this.middlewares.slice(0).reverse().forEach(function(middleware) {
                if (middleware.response)
                    middleware.response(method, response);
            });
